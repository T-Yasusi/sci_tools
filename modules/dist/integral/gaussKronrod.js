...
const MAX_LOOP = 1000;
export default function gaussKronrod(f, x0, x1, tol = 1.0e-8) {
    let counter = 0;
    ...
    while (abs(sub(gauss_val, kronrod_val)) > tol) {
        counter++;
        if (counter >= MAX_LOOP)
            throw new Error(`!!! Integral by Gauss Kronrod over Max Iteration : ${MAX_LOOP} !!!`);
...
function calc(f, x0, x1) {
    ...
    let gauss_val = 0;
    for (let i = 0; i < gauss_calc_points.length; i++)
        gauss_val = add(gauss_val, mul(gauss_weights[i], f(gauss_calc_points[i])));
    let kronrod_val = 0;
    for (let i = 0; i < kronrod_calc_points.length; i++)
        kronrod_val = add(kronrod_val, mul(kronrod_weights[i], f(kronrod_calc_points[i])));
