...
export default function associatedLegendre(m: number, l: number, x: InputType): number | Complex {
  ...
  let a = 1;
  for (let i = 1; i <= sub(mul(2, m2), 1); i = add(i, 2)) a = mul(a, i);
  let a2 = 0;
...
function parityFactor(l: number, m: number): number {
  if (m >= 0) return 1;
  let deno = 1;
  for (let i = add(add(l, m), 1); i <= sub(l, m); i++) deno = mul(deno, i);
  if (mod(m, 2) === 0) return div(1, deno);else return div(neg(1), deno);
}