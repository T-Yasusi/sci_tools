...
export default function normalizedAssociatedLegendre(m: number, l: number, x: InputType): number | Complex {
  ...
  if (m2 > l) return 0;
  if (l === 0 && m2 === 0) return div(1, sqrt(2));
  let a2 = 0;
  let a = div(add(mul(2, m2), 1), 2);
  for (let i = 1; i <= mul(2, m2); i++) a = div(a, i);
  a = sqrt(a);
  for (let i = sub(mul(2, m2), 1); i > 0; i = sub(i, 2)) a = mul(a, i);
  let a1 = mul(a, pow(sub(1, mul(x, x)), mul(0.5, m2)));
  if (m2 === l) return m2 === m ? a1 : mul(pow(neg(1), m2), a1);
