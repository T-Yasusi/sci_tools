...
export default ( data_x: number[], data_y: number[], par: number[], func: (...x: number[])=> number, tol: number = 10e-3 ): object => {
	...
	const g_par: number[] = new Array(simplex[0].parameter.length).fill(0);
        ...
        const r_par: number[] = new Array(simplex[0].parameter.length).fill(0);
        for( let j=0; j<simplex[0].parameter.length; j++ ) r_par[j]=2*g_par[j]-simplex[simplex.length-1].parameter[j];
        const r_point: { chi2: number, parameter: number[] } = { 'parameter': r_par, 'chi2': calcChi2(data_x, data_y, r_par, func) };

	if( simplex[0].chi2>r_point.chi2 ){
            const e_par: number[] = new Array(simplex[0].parameter.length).fill(0);
            for( let j=0; j<simplex[0].parameter.length; j++ ) e_par[j]=2*r_par[j]-g_par[j];
            const e_point: { chi2: number, parameter: number[] } = { 'parameter': e_par, 'chi2': calcChi2(data_x, data_y, e_par, func) };
            ...
            const c_par: number[] = new Array(simplex[0].parameter.length).fill(0);
            for( let j=0; j<simplex[0].parameter.length; j++ ) c_par[j]=0.5*simplex[simplex.length-1].parameter[j]+0.5*g_par[j];
            const c_point: { chi2: number, parameter: number[] } = { 'parameter': c_par, 'chi2': calcChi2(data_x, data_y, c_par, func) };
