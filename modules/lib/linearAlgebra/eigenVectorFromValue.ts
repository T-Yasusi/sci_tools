...
export default function eigenVectorFromValue(val: number | Complex, mat: Matrix | ComplexMatrix): Vector | ComplexVector {
  ...
  for (let i = 0; i < mat.cols; i++) {
    ...
    for (let j = 0; j < mat.rows; j++) A[i][j] = div(A[i][j], a);
    for (let j = add(i, 1); j < mat.cols; j++) {
      const b = div(A[j][i], A[i][i]);
      for (let k = 0; k < mat.cols; k++) A[j][k] = sub(A[j][k], mul(b, A[i][k]));
    }
  }
  ...
  for (let i = 0; i < zeroRow; i++) result[i] = neg(A[i][zeroRow]);
  if (sub(mul(mat, result), mul(val, result)).norm() > THRESHOLD) throw new Error('eigenVector from Value can not calculated');
